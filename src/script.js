...
const material = new THREE.MeshBasicMaterial({ color: 0xd0d0d0, opacity: .7, transparent: true })
...
const parameters = {
    ...
    textureIndex: 0,
    spin: () => {
        gsap.to(group.rotation, { y: group.rotation.y + Math.PI * 2, duration: 1 });
    }
}
...
gui.add({ reset: () => location.reload() }, 'reset').name('Reset');
